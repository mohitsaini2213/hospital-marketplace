    ...
    <div className="flex min-h-screen bg-[var(--color-paper)]">
      <aside className="hidden w-64 shrink-0 flex-col border-r border-[var(--color-line)] bg-white lg:flex">
            ...
            <NavLink
              ...
              className={({ isActive }) =>
                `flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition-colors ${
                  isActive ? 'bg-[var(--color-teal-050)] text-[var(--color-teal-700)]' : 'text-[var(--color-ink-soft)] hover:bg-[var(--color-paper-dim)] hover:text-[var(--color-ink)]'
                }`
              }
            >
              <Icon size={15} /> {label}
        ...
        <div className="border-t border-[var(--color-line)] p-3">
          <div className="mb-2 truncate px-3 text-xs text-[var(--color-ink-soft)]">{user?.email}</div>
          <button
      ...
      <div className="flex-1">
        ...
        <div className="flex items-center justify-between border-b border-[var(--color-line)] bg-white px-4 py-3 lg:hidden">
          <Link to="/" className="font-display text-base font-semibold text-[var(--color-teal-900)]">
            Hospital Marketplace
          </Link>
          <button onClick={handleLogout} className="text-sm font-medium text-[var(--color-red-600)]">
            Logout
          </button>
        </div>
