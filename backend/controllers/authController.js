...
const registerFacility = asyncHandler(async (req, res) => {
  ...
  const tpl = templates.registrationReceived(
    facility.ownerName
  );
  ...
  res.status(201).json({
    success: true,
    message:
      'Registration submitted successfully. Your listing is under verification.',
    data: {
      id: facility._id,
...
const loginAdmin = asyncHandler(async (req, res) => {
  ...
  const match = await admin.comparePassword(
    password
  );
...
const refresh = asyncHandler(async (req, res) => {
  ...
  const account = await Model.findById(
    decoded.id
  );
  ...
  // Prevent disabled admins from refreshing
  // an existing session.
  if (
    decoded.type === 'admin' &&
    !account.isActive
...
const logout = asyncHandler(async (req, res) => {
  ...
  // exact same cookie attributes used when it
...
const getMe = asyncHandler(async (req, res) => {
  if (req.facility) {
    return res.json({
      success: true,
      data: req.facility.toPublicJSON(),
      type: 'facility',
    });
  }
...
const forgotPassword = asyncHandler(async (req, res) => {
  ...
  const link =
    `${process.env.CLIENT_URL}/reset-password/${rawToken}?type=${
      accountType === 'ADMIN'
        ? 'ADMIN'
        : 'FACILITY'
    }`;
...
// Authenticated facility or admin
const changePassword = asyncHandler(async (req, res) => {
  ...
  const match = req.admin
    ? await withHash.comparePassword(
        currentPassword
      )
